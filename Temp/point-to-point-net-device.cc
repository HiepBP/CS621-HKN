  ...
  else 
    {
      ...
      if(m_is_router){
        ...
        if (header.GetProtocol() == (int)0x4021) {
          ...
          int num = packet->RemoveHeader(ip_header);
          size = size - num;
          ...
          num = packet->RemoveHeader(udp_header);
          size = size - num;
          std::cout<<std::dec<<num<<std::endl;
          //Get the data
          packet->CopyData(buffer, size);
          ...
          uint8_t *decompress_buffer = new uint8_t[size];
          uLongf data_size = number_of_bytes + 2;
          uncompress(decompress_buffer, &data_size, &buffer[0], size);
          std::cout<<std::dec<<data_size<<std::endl;
          decompress_buffer = &decompress_buffer[2];
          data_size = data_size - 2;

          // Packet *packet_data = new Packet(decompress_buffer, data_size);
          // std::cout<<"FINISH"<<std::endl;
          // packet->RemoveAtEnd(10000000);
          // packet->AddAtEnd(packet_data);
          std::cout<<std::dec<<"Uncompress: "<<data_size<<":";
          packet = new Packet(decompress_buffer, data_size);
          std::cout<<" -Create packet from buffer data"<<std::endl;
          //Add udp size
          size = data_size + 8;
          udp_header.ForcePayloadSize(size);
          packet->AddHeader(udp_header);
          std::cout<<" -Add UDP Header back"<<std::endl;
          //Add ip size
          ip_header.SetPayloadSize(size);
          packet->AddHeader(ip_header);
          std::cout<<" -Add IP Header back"<<std::endl;

          //Update protocol to 0x0021
          header.SetProtocol(0x0021);
          std::cout<<" -Set Protocol to 0x0021"<<std::endl;
        }
        packet->AddHeader (header);
      } else {
        std::cout<<"Client/Server receive packet"<<std::endl;
      }
  ...
  uint16_t protocolNumber)
{
  ...
  if(m_is_router){
    ...
    if (header.GetProtocol() == (int)0x0021)
    {
      ...
      int data_size = packet->GetSize();
      uLongf size = data_size + 2;
      uint8_t *buffer = new uint8_t[size];
      buffer[0] = 0x00;
      buffer[1] = 0x21;
      ...
      // std::string compressed_data = zlib_compress_string("abc",9);
      // std::cout<<"Len: "<<compressed_data.length()<<" - Compress data: "<<compressed_data<<std::endl;

      uint8_t *compress_buffer = new uint8_t[size];
      uLongf new_size;
      compress2(compress_buffer, &new_size, buffer, size,9);
      std::cout<<"Compressed: "<<new_size<<":";
      for (int i = 0; (unsigned)i < new_size; ++i)
    ...
    std::cout<<"Client/Server send packet"<<std::endl;
  }
