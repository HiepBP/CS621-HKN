  ...
  else 
    {
      if(m_is_router){
        ...
        if (header.GetProtocol() == (int)0x4021) {
          ...
          int len = packet -> RemoveHeader(ip_header);
          
          size = size - len;
          UdpHeader udp_header;
          len = packet -> RemoveHeader(udp_header);
          size = size - len;
          ...
          //Update the packet
          packet = new Packet(decompress_buffer, data_size);//packet will be clear at the end
          //Update udp size
          int udp_size = data_size + 8;
  ...
  uint16_t protocolNumber)
{
  ...
  if(m_is_router){
    ...
    if (header.GetProtocol() == (int)0x0021)
    {
      ...
      ip_header.SetTtl(ip_header.GetTtl() - 1);
      packet->AddHeader(ip_header);
      //Update protocol to 0x4021
